<h1 align="center">Hi 👋, I'm Sujan Kowshik</h1>
<h3 align="center">Aspiring Software Engineer | Full-Stack Developer | Problem Solver</h3>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=sujankowshik&label=Profile%20views&color=0e75b6&style=flat" />
</p>

---

## 🚀 About Me
🎓 Computer Science Undergraduate at **Anurag University** (CGPA: **9.52**)  
💻 Passionate about **Software Engineering, Full-Stack Development & Applied AI**  
🏆 Multiple **hackathon winner & finalist**  
🧠 Strong foundation in **DSA, OS, DBMS, CN**

---

## 🛠️ Tech Stack
<p align="left">
  <img src="https://skillicons.dev/icons?i=java,python,javascript,c,react,nodejs,html,css,mysql,firebase,git" />
</p>

---

## 📌 Featured Projects
- 🛒 **Full-Stack eCommerce Platform** – Auth, Payments, Admin Dashboard  
- 📊 **Financial Portfolio Dashboard** – Market APIs & Data Visualization  
- 🏠 **House Price Prediction** – Flask-based ML app (96% accuracy)

👉 Check my pinned repositories below 👇

---

## 📊 GitHub Activity Overview
<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=sujankowshik&theme=github_dark" />
</p>

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=sujankowshik&theme=github_dark" height="160"/>
</p>



---

## 📈 Most Used Languages
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=sujankowshik&layout=compact&theme=tokyonight"/>
</p>

---

## 🧠 Problem Solving
- Solved **150+ DSA problems** on LeetCode, Codeforces & GFG  
- Focus: Arrays, Strings, Trees, Graphs, Dynamic Programming  
- Practicing **interview-oriented problem solving daily**

---

## 🏆 Achievements
🥇 1st Place — Tejas 2K25  
🥈 2nd Place — Hackverse  
⭐ Top Performer — Visionnova  
🔟 Top 10 — Udhgam Hackathon (Woxsen University)

---

## 🎯 Currently Working On
- Improving **DSA & Core CS fundamentals**
- Building **weekly projects** with clean architecture
- Enhancing **communication & interview skills**

---

## 📫 Connect With Me
<p align="left">
  <a href="https://www.linkedin.com/in/sujankowshikjilla" target="_blank">
    <img src="https://skillicons.dev/icons?i=linkedin" width="45" />
  </a>
  &nbsp;&nbsp;
  <a href="mailto:sujankowshikjilla@gmail.com">
    <img src="https://skillicons.dev/icons?i=gmail" width="45" />
  </a>
</p>


---

⭐ If you like my work, consider exploring my repositories!
